...
// Simplified image source - just use the product image or fallback
const getImageSrc = (product: Product) => {
    if (imageError.value) {
        return (
            product.fallbackImage ||
            `https://via.placeholder.com/400x300/3B82F6/FFFFFF?text=${encodeURIComponent(product.name)}`
        );
    }
...
<template>
    ...
    >
        ...
        <div class="relative overflow-hidden bg-gray-50">
            ...
            <div
                v-if="!isImageLoaded"
                class="w-full h-48 bg-gradient-to-r from-gray-200 via-gray-100 to-gray-200 animate-pulse"
            ></div>
            ...
            <img
                ...
                :class="{
                    'opacity-0': !isImageLoaded && !imageError,
            ...
            <!-- Quick view badge -->
            <div
                class="absolute top-3 right-3 bg-white/90 backdrop-blur-sm rounded-full px-3 py-1 text-xs font-medium text-gray-700 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
            >
                Quick Add
            </div>
            ...
            <button
                class="absolute top-3 left-3 p-2 bg-white/90 backdrop-blur-sm rounded-full text-gray-400 hover:text-red-500 hover:bg-white transition-all duration-200 opacity-0 group-hover:opacity-100 shadow-sm"
            >
                <svg
                    class="w-4 h-4"
                    fill="none"
